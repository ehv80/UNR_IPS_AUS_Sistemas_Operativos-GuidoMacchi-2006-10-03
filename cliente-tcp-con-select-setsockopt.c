...
int main(int argc, char **argv){
	...
	/* Por 'sock' va a iniciar conexion TCP
	 * a un servidor TCP remoto */
	...
	fd_set in, in_orig;
	/* Tiempo limite que debe transcurrir antes que
	 * select(2) retorne */
	struct timeval tv;
	...
	/* 'sock' inicia la conexion TCP hacia 
	 * la dirección remota 'sin.sin_addr' */
	if( connect(sock, (sad)&sin, sizeof sin)<0)
	    error("conect");
	
	/* Tiene select(2) */
	/* Limpia el conjunto de descriptores de
	 * fichero 'in_orig' */
	FD_ZERO(&in_orig);
	/* Añade al conjuto 'in_orig' el 
	 * descriptor STDIN */
	FD_SET(0, &in_orig);
	/* Añade al conjunto 'in_orig' el 
	 * descriptor 'sock' */
	FD_SET(sock,&in_orig);
	
	/* Tiene 1 hora */
	/* Tiempo hasta que select(2) 
	 * regrese: 3600 segundos */
	tv.tv_sec=TIME;
	tv.tv_usec=0;
	
	for(;;){
		/* Copia contenido del conjunto 
		 * 'in_orig' en 'in' */
		 ...
		 * que select(2) retorne expira */
		if(cuanto==0)
			error("timeout");
		/* averiguamos donde hay algo para leer*/
		// Si hay para leer desde STDIN
		if(FD_ISSET(0,&in)){
			/* Lee hasta 1024 caracteres 
			 * de STDIN y los pone en 'linea' */
		...
		if(FD_ISSET(sock,&in)){
			/* Lee hasta 1024 caracteres 
			 * de 'sock' y los pone en 'linea' */
			cuanto = read(sock, linea, SIZE);
			/* Si lectura devuelve < 0 */
			if(cuanto < 0)
				error("read"); // Error al leer
			else if(cuanto==0) break;	
			/* Si lectura devuelve 0
			 * entonces parar la ejecucion */
			// Marca el final del buffer con '0'
			linea[cuanto]=0;
